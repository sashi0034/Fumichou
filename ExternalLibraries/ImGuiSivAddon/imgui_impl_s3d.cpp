...
{
	const auto itr = KeyId2ImGuiKeyDic.find(input.code());
...
static void RenderImeWindow()
{
	...
	const ImeWindowData& imeWindow = g_context->imeWindow.value();
	...
	(void)drawableText.region(size, imeWindow.pos).draw(Palette::White);

	(void)drawableText.draw(size, imeWindow.pos, Palette::Black);
}
...
bool ImGui_ImplS3d_Init()
{
	...
	const auto fontScale = 1.25 / Window::GetState().scaling;
...
void ImGui_ImplS3d_NewFrame()
{
	...
	// Display
	{
		...
		io.DeltaTime = static_cast<float>(Scene::DeltaTime());
	}
	...
	for (const Input& mouseInput : Mouse::GetAllInputs())
	{
...
void ImGui_ImplS3d_RenderDrawData(ImDrawData* draw_data)
{
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++)
	{
		...
		const ScopedRenderStates2D r(rasterizer);
