...
public:
	template <int table>
	static MappedRead mapReadNametable(const Ppu& ppu, bool isMirror)
	{
		...
		return MappedRead{
			...
			{
				const auto& ppu = *static_cast<const Ppu*>(ctx);
				addr &= 0x03FF;
				return ppu.m_nametableData[addr + ppu.m_nametableOffset[table]];
			}
		};
	}
...
namespace Nes
{
	MappedRead Ppu::In::Mm::MapReadChr(const Hardware& hw, addr16 addr)
	{
		...
		if (AddrRange<addr16>(0x2000, 0x3EFF).IsBetween(addr))
		{
			...
			{
			case 0: return Impl::mapReadNametable<0>(ppu, isMirror);
			case 1: return Impl::mapReadNametable<1>(ppu, isMirror);
			case 2: return Impl::mapReadNametable<2>(ppu, isMirror);
			case 3: return Impl::mapReadNametable<3>(ppu, isMirror);
			default: break;
			}
		}
