...
namespace
{
	...
	{
		return Util::GetTomlStyle<T>(U"GuiScreenOverlay." + key);
	}

	constexpr int modeCount_2 = 2;
}

struct GuiScreenOverlay::Impl
{
	...
	void Update(const update_args& args)
	{
		...
		const auto tlRect = RectF(Arg::center = tlPoint, SizeF{1.0, 0.8} * iconSize).rounded(8).draw(ColorF{0.1, 0.5});
		...
		const auto trRect = RectF(Arg::center = trPoint, SizeF{1.0, 0.8} * iconSize).rounded(8).draw(ColorF{0.1, 0.5});
...
namespace Gui
{
	GuiScreenOverlay::GuiScreenOverlay() :
		p_impl(std::make_shared<Impl>())
	{
	}

	void GuiScreenOverlay::Update(const update_args& args)
	{
		p_impl->Update(args);
	}
}
